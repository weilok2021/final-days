...
## What this is
The browser-extension port of Final Days for Chrome and Edge (Manifest V3, TypeScript): the 4 px life bar on every web page and the once-a-day countdown. It exists because the Go exe was flagged by SentinelOne on the managed work laptop; nothing here runs outside the browser.
...
- Content scripts re-render at the next boundary (a quiet-hours edge or local midnight, from nextChange in life.ts) instead of on a fixed interval, and again on visibility and focus, so a laptop waking up catches up at once. A timer never triggers a countdown: at midnight the bar updates and the countdown waits for the next "coming back" event, as on Windows.
- Settings (birth, strip, countdown, quietHours) live in chrome.storage.sync; state (lastCountdown) in chrome.storage.local. Lifespan is a constant in life.ts.
- The popup is the tray-menu equivalent: days left, day count, Show the countdown, the Life bar switch, Options. The switch writes the setting, so it persists, unlike the Windows hotkey which is per session; the browser has no session to map that to.
...
- `src/content.ts`: strip, hover label and countdown overlay, each in a closed shadow root on a custom element (`final-days-strip`, `final-days-countdown`) whose host styles are inline `!important` so page CSS cannot hide or move them.
...
4. Because it is the first page today, the countdown appears: dark full-tab overlay, the days-left number very large, "days left · day N of 29,220", the question, "click anywhere to continue". Click or press a key: gone. Reload, open other pages, switch tabs: no second countdown today.
5. Pin the toolbar icon (extensions menu). Expect the dark tile with the bar, tooltip "Final Days · X days left". The popup shows the number, the day count, Show the countdown (shows it again now), the Life bar switch (removes and restores the bar on every tab) and Options. On chrome://extensions or the new tab, Show the countdown must explain it needs a web page.
...
8. Lock (Win+L) and unlock with a web page in front: no countdown, it was shown today. Reset the state (console line above), lock and unlock again: the countdown appears on that page within a few seconds. Once more: nothing.
9. Reset the state, then restore a window with several tabs (Ctrl+Shift+T) or start the browser with tabs restored: exactly one tab shows the countdown.
10. No bar on chrome://extensions, the new tab page, the Chrome Web Store or a PDF, and no errors on the card. Full-screen video: the bar is not over it; leaving fullscreen brings it back. Typing in a page works normally; keys are only swallowed while a countdown is showing.
11. Reset the state, then open a page that redirects on arrival (for example a site's old address that forwards to a new one): the countdown appears on the page you land on, not lost on the one that went away.
...
13. Sites mode: choose "Every time you open one of these sites", enter `youtube.com, facebook.com`, Save. Open GitHub: bar, no countdown. Open YouTube: countdown; click it; switch tabs and back: no repeat; reload: countdown again. Open Facebook: countdown. Switch back to "Once a day": the old rule returns.
14. Seen rule: reset the state, open a listed site, wait 4 seconds, then go to another listed site by typing its address while the countdown is still up: no countdown on the next page. Reset, do the same but leave within a second: the countdown reappears on the next page.

## Loose ends
- Loading from the `\\wsl.localhost` path works (confirmed in Chrome on the work PC, 2026-09-03), as do the options page, the strip, the hover label and the countdown. The end-to-end suite passes; the desktop-only checklist items (lock and unlock, toolbar, shortcut) are still to run by hand.
...
- After the extension is reloaded, disabled or removed, a page that was already open keeps its last drawn bar until it next tries to talk to the worker (a tab switch, window focus, or the midnight or quiet-hours timer), then takes it down. Until reloaded, such a page cannot show the countdown; the popup says so.
...
- Rename "countdown" to "countdown" everywhere: copy, code, settings and storage keys, docs, tests. Whether the shared spec and the Windows tray menu string follow is the first question for the next session.
