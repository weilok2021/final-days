...
## What this is
Final Days, version 1 for Windows: a 4-pixel life bar reserved along the top screen edge, plus a once-a-day full-screen countdown at first unlock showing the days left. Portable single exe, Go, no installer.
...
- Scope of v1: A (edge strip) + B (daily countdown). C (wallpaper grid) and D (friction gate / app-block) deferred.
...
- Countdown: first unlock of each calendar day (and first launch of the day), black full screen, days-left number large, line "days left · day N of 29,220", question "Is today worth one of your remaining N days?", click to dismiss. Never more than once a day.
...
## Open questions
- None blocking. Wording variation for the countdown (small rotating set) is a maybe for later.
...
3. Within ~1 s of start the full-screen countdown appears (days left, "Is today worth one of your remaining N days?"). Click or press a key to dismiss. `final-days.state` now holds today's date; restarting the exe must not show it again today.
4. Tray icon (may be in the overflow): right-click → menu with days left, Show the countdown, Life bar (checked), Start with Windows, Open config file, Quit. Left double-click shows the countdown.
...
7. Lock the PC (Win+L), unlock: no countdown (already shown today). Delete `final-days.state`, lock and unlock: countdown appears ~1.5 s after unlock.
