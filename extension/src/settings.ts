...
import { parseBirth, parseSiteList, type CalendarDate } from './life.ts';
...
/** Settings that went with the page strip on 2026-09-04; the worker deletes them when it starts. */
export const REMOVED_SYNC_KEYS: readonly string[] = ['strip', 'quietHours'];
...
/**
 * Turns stored text into a date and sites. Null while the date of birth is
 * unset or unusable. A broken site list means no site at all, so a damaged
 * list can never make the countdown appear where it was not wanted.
 */
